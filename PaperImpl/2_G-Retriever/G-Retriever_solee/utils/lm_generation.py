# Language model utils for step 3 (generation)
...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if args.llm_frozen == 'True':
            ...
            lora_r: int = 8 # rank
            lora_alpha: int = 16 # scaling factor of LoRA
            lora_dropout: float = 0.05 # dropout rate
            lora_target_modules = [ # use them for query and value projection
                "q_proj",
                "v_proj",
            ]
            config = LoraConfig(
    ...
    def forward(self, samples):
        ...
        bos_embeds = self.tokenizer(BOS, add_special_tokens=False, return_tensors='pt').input_ids[0]
        pad_embeds = self.word_embedding(torch.tensor(self.tokenizer.pad_token_id)).unsqueeze(0)

        # encode graphs
        graph_embeds = self.encode_graphs(samples) # TODO: Implement GNN
        graph_embeds = self.projector(graph_embeds)

        batch_size = len(samples['id'])
        
        # initialize input embeddings / attention mask / label_input_ids
        batch_inputs_embeds = []
