...
# Implementation for planning module
# This module aims to optimize 1/|Z*| \sum_{z \in Z*} log \prod_{i=1}^|z| P_\theta(r_i | r_{<i}, q)
# That is, given the question q, it generates output of possible paths that will be used in retrival-reasoning module
# the output z has a format of z = <PATH> r_1 <SEP> r_2 <SEP> ... <SEP> r_l </PATH>
...
def generate_paths(args):
    ...
    if args.lora or os.path.exists(args.model_path + "/adapter_config.json"):
        ...
        model = AutoModelForCausalLM.from_pretrained(
            args.model_path, device_map="auto", torch_dtype=torch.bfloat16, use_auth_token=True
        )
    ...
    prediction_file = os.path.join(output_path, f"prediction_{args.n_beam}_{args.do_sample}.jsonl")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", type=str, default='rmanluo/RoG') # meta-llama/Llama-2-7b-chat-hf
    ...
    parser.add_argument("--outsource_path", type=str, default="results/1_planning")
